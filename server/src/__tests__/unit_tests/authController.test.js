...
describe('registerUser', () => {
    ...
    beforeEach(() => {
        req = { body: { name: 'testUser', email: 'testUser@web.de', password: '1111' } };
        res = { status: jest.fn().mockReturnThis(), json: jest.fn(), send: jest.fn() };
        jest.clearAllMocks();
    });

    it('should return access and refresh tokens when registration is successful', async () => {
        const mockUser = { id: '12345', name: 'testUser', email: 'testUser@web.de', password: '1111', save: jest.fn() };
        ...
        expect(User).toHaveBeenCalledWith({ name: 'testUser', email: 'testUser@web.de', password: '1111' });
        expect(bcryptjs.hash).toHaveBeenCalledWith('1111', 'salt');
        expect(jwt.sign).toHaveBeenCalledWith({ id: '12345' }, process.env.JWT_SECRET, { expiresIn: 3600 } );
        expect(RefreshToken.create).toHaveBeenCalledWith({ token: 'refreshToken', userId: '12345', expiresAt: expect.any(Date) });
...
describe('loginUser', () => {
    ...
    it('should return access and refresh tokens when login is successful', async () => {
        const mockUser = { id: '12345', name: 'testUser', email: 'testUser@web.de', password: '1111' };
...
describe('refreshToken', () => {
    ...
    it('should refresh access and refresh tokens and return them', async () => {
        const mockValidRefreshTokenDoc = { userId: 'existingUser', expiresAt: new Date(Date.now() + 60 * 1000), revoked: false }
        ...
        expect(RefreshToken.findOne).toHaveBeenCalledWith({ token: 'refreshToken' });
        expect(User.findById).toHaveBeenCalledWith('existingUser');
        expect(jwt.sign).toHaveBeenCalledWith({ id: 'existingUser' }, process.env.JWT_SECRET, { expiresIn: 3600 } );
        expect(RefreshToken.create).toHaveBeenCalledWith({ token: 'newRefreshToken', userId: 'existingUser', expiresAt: expect.any(Date) });
        expect(RefreshToken.updateOne).toHaveBeenCalledWith({ token: 'refreshToken' }, { revoked: true });
        expect(res.status).toHaveBeenCalledWith(200);
    ...
    it('should return 401 if refresh token is expired', async () => {
        RefreshToken.findOne.mockResolvedValue({ expiresAt: new Date(Date.now() - 60 * 1000), revoked: false });
    ...
    it('should return 403 if no user is associated with refresh token', async () => {
        RefreshToken.findOne.mockResolvedValue({ userId: 'nonExistingUser', expiresAt: new Date(Date.now() + 60 * 1000), revoked: false });
